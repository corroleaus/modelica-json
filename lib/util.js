 ...
 * the class's own identity.
 *
 * A dotted name is NOT necessarily qualified. Modelica resolves a type by
 * walking outwards through enclosing packages, so a class may name a partial
 * path from any level: G36's TrimAndRespond declares
 * `CDL.Reals.Sources.Constant resAmoCon`, where `CDL` resolves to
 * `Buildings.Controls.OBC.CDL` from inside `...OBC.ASHRAE.G36.Generic`.
 * Treating every dotted name as already qualified emitted that verbatim, which
 * names a class that does not exist and which a consumer cannot tell apart
 * from a real dependency. `searchPath` locates such a name; it was simply
 * never asked.
 *
 * The qualified name is built from the class file itself — its `within` plus
 * its basename — rather than by appending the specifier, which would repeat
 * the path segments for a dotted one. This is idempotent: an
 * already-qualified name resolves to the same file and back to itself.
 *
 * Names that cannot be resolved, and classes defined inside a `package.mo`,
...
function resolveTypeSpecifier (typeSpecifier, within, sourceMoFile) {
  ...
  if (moFiles !== undefined && moFiles !== null && moFiles.length > 0 &&
    ...
    const className = path.basename(moFiles[0], '.mo')
    if (classWithin !== null && classWithin.length > 0) {
      resolved = `${classWithin}.${className}`
    }
  }
 ...
 * name.
 *
 * `resolveTypeSpecifier` applies the same partial-qualification rule, and once
 * could not be used here because it returned early for any name containing a
 * '.'. That early return is gone. The two still stay separate: this one must
 * also decide WHETHER the located class is an enumeration, and must cope with
 * a file declaring several of them — neither of which a component type needs.
 *
