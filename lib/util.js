...
/**
 * Check whether a Modelica file declares `className` as an enumeration type.
 *
 * @param moFile path to the .mo file
 * @param className name of the class the file is expected to declare
 * @returns {boolean} true if the file declares `type <className> = enumeration(`
 */
function isEnumerationDeclaration (moFile, className) {
...
function resolveEnumerationType (typeSpecifier, within, sourceMoFile) {
  ...
  if (moFiles !== undefined && moFiles !== null && moFiles.length > 0) {
    ...
    if (classWithin !== null && classWithin.length > 0 && isEnumerationDeclaration(moFile, className)) {
      resolved = { name: `${classWithin}.${className}`, within: classWithin }
    }
  }
