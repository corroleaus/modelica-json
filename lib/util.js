...
function resolveEnumerationType (typeSpecifier, within, sourceMoFile) {
  ...
  let resolved = null
  let moFiles = searchPath([typeSpecifier], within, sourceMoFile)
  if (moFiles === undefined || moFiles === null || moFiles.length === 0) {
    // searchPath walks down from each MODELICAPATH entry, so it only finds the
    // class when MODELICAPATH holds the directory *containing* the library. It
    // finds nothing when MODELICAPATH points at the library directory itself
    // (`…/modelica-buildings/Buildings`), which is an equally common
    // convention. getMoFiles matches by path suffix instead, so it still
    // resolves the class there.
    try {
      moFiles = getMoFiles(typeSpecifier).filter(
        (f) => f.endsWith('.mo') && fs.existsSync(f))
    } catch (e) {
      moFiles = []
    }
  }
  if (moFiles !== undefined && moFiles !== null && moFiles.length > 0) {
    const moFile = moFiles[0]
    ...
    if (classWithin !== null && classWithin.length > 0 && content !== null &&
        isEnumerationDeclaration(content, enumerationName)) {
      ...
      // The reconstruction assumes the enumeration sits directly in the class
      // the file declares. That does not hold for a monolithic package.mo
      // holding nested packages, where searchPath resolves to the outermost
      // file: Modelica.Media.Interfaces.Choices.ReferenceEnthalpy lands in
      // Modelica/Media/package.mo and would be rebuilt as
      // Modelica.Media.ReferenceEnthalpy. Requiring the result to end with the
      // specifier as written rejects those rather than emitting a wrong IRI.
      if (name === typeSpecifier || name.endsWith(`.${typeSpecifier}`)) {
        resolved = { name, within: name.slice(0, name.lastIndexOf('.')) }
      }
    }
  }
