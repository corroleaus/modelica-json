...
function generateCxf (jsonOutput, moFile, tempDir, prettyPrint, generateElementary, generateCxfCore, mode) {
  ...
  if (fileNameTokens.includes('CDL')) {
    ...
    if ((isElementaryBlock && (generateCxfCore || generateElementary)) || (!isElementaryBlock)) {
      ...
      cxfGraphJsonLd = Object.assign({}, cxfGraphJsonLd, cxfExt.getCxfGraph(instances, requiredReferences, blockName, generateElementary, generateCxfCore, tempDir))
    }
    ...
    const requiredReferences = jsonOutput.requiredReferences
    const blockName = moFile.split(path.sep)[moFile.split(path.sep).length - 1].split('.mo')[0]
    const cxfGraphJsonLd = cxfExt.getCxfGraph(instances, requiredReferences, blockName, generateElementary, generateCxfCore, tempDir)
