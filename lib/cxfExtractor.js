      ...
      Object.keys(connections).forEach(fromElement => {
        ...
        toElements.forEach(toElement => {
          ...
          getConnectionPairs(fromElements, expandedToElements).forEach(([from, to]) => {
            const fromNode = getInstanceNode(graph, from, newInstances, s231Ns, cxfPrefix)
            const toNode = getInstanceNode(graph, to, newInstances, s231Ns, cxfPrefix)
...
function getInstanceNode (graph, element, newInstances, s231Ns, cxfPrefix) {
  ...
  if (elementParts.subInstance === '' && elementParts.instanceExpression === elementParts.instance) {
    return newInstances[elementParts.instance].cxfNode
  }
  const instanceNode = newInstances[elementParts.instance].cxfNode
  const instancePackage = instanceNode.value.split('.').slice(0, -1).join('.').split('#')[1]
  const instanceName = elementParts.instanceExpression.split('.').slice(-1)[0]
  const prefix = instanceNode.prefix
  const nodeName = elementParts.subInstance === ''
    ? `${instancePackage}.${instanceName}`
    : `${instancePackage}.${instanceName}.${elementParts.subInstance}`
  if (prefix === 's231') {
    node = s231Ns(nodeName)
  } else {
    node = cxfPrefix(nodeName)
  }
